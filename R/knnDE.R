...
knnDE <-
  function(X, Grid, k){
    ...
    # ensure input X is a matrix
    if (!is.numeric(X) && !is.data.frame(X)) {
      stop("X should be a matrix of coordinates")
    }
    # ensure input Grid is a matrix
    ...
    # function in its final form, multiplying number of points in X by the volume of unit ball
    # by the Euclidean distances from k nearest neighbors to the power of d dimensions considered
    out <- k / (n * v.d * r.k ^ d)
    return(out)
  }
